...
main(void)
{
	...
	bdd p[100][4], q[100][4], et[100], cet[100];
	...
	if (!input.is_open())
		printf("File open failed! Please check  your file name\n");
	else
	{
		...
		for (i = 0; i < varnum; i++)
		{
			...
			{
				if (m % 2 == 0)
					pninit = bdd_apply(pninit, bdd_not(p[i][j]), bddop_and);
				else
					pninit = bdd_apply(pninit, p[i][j], bddop_and);
				m = m / 2;
			}
		}
