...
double f;
 ...
 bdd p[varnum][bvar],q[varnum][bvar],et[100],cet[100];
 bdd_init(100000,10000);
bdd_setvarnum(2*bvar*varnum);
for(i=0;i<varnum;i++)
for(j=0;j<bvar;j++)
{p[i][j]=bdd_ithvar(i*bvar+j);
q[i][j]=bdd_ithvar((i+varnum)*bvar+j);
}
...
for(i=0;i<varnum;i++)
 ...
 int oldvar[varnum][bvar];
int newvar[varnum][bvar];
for(i=0;i<varnum;i++)
for(j=0;j<bvar;j++)
{oldvar[i][j]=i*bvar+j+bvar*varnum;
newvar[i][j]=i*bvar+j;
}
  ...
  for(i=0;i<tnum;i++)
     ...
     bdd add[varnum],reduce[varnum],a[bvar];
     for(i=0;i<varnum;i++)
         {
	 reduce[i]=bddtrue;
	 ...
	 a[4]=bdd_apply(bdd_not(p[i][3]),bdd_apply(bdd_not(p[i][2]),bdd_apply(bdd_not(p[i][1]),bdd_not(p[i][0]),bddop_and),bddop_and),bddop_and);
	 a[4]=bdd_apply(a[4],p[i][4],bddop_xor);
	 a[4]=bdd_apply(a[4],q[i][4],bddop_biimp);
	 for(j=0;j<bvar;j++)
	     {
	 ...
	 a[3]=bdd_apply(a[3],p[i][3],bddop_xor);
	 a[3]=bdd_apply(a[3],q[i][3],bddop_biimp);
	 a[4]=bdd_apply(p[i][3],bdd_apply(p[i][2],bdd_apply(p[i][1],p[i][0],bddop_and),bddop_and),bddop_and);
	 a[4]=bdd_apply(a[4],p[i][4],bddop_xor);
	 a[4]=bdd_apply(a[4],q[i][4],bddop_biimp);
	 for(j=0;j<bvar;j++)
	     {
  ...
  for(i=0;i<tnum;i++)
      {
	   ...
	   bdd_setpairs(pair,oldvar[tpre[i][j]-1],newvar[tpre[i][j]-1],bvar);
	   for(k=0;k<bvar;k++)
	  ...
	  pnnew=bdd_apply(pnnew,add[tpos[i][j]-1],bddop_and);
	   bdd_setpairs(pair,oldvar[tpos[i][j]-1],newvar[tpos[i][j]-1],bvar);
	   for(k=0;k<bvar;k++)
...
for(k=0;k<2*bvar*varnum-bvar*varnum;k++)
    pn=bdd_apply(pn,bdd_ithvar(k+bvar*varnum),bddop_and);
f=bdd_satcount(pn);
fprintf(fp,"\n pn is %f states:\n",f);
...
for(k=0;k<2*bvar*varnum-bvar*varnum;k++)
     pndead=bdd_exist(pndead,bdd_ithvar(k+bvar*varnum));
...
for(k=0;k<2*bvar*varnum-bvar*varnum;k++)
    pngood=bdd_apply(pngood,bdd_ithvar(k+bvar*varnum),bddop_and);
...
for(k=0;k<bvar*varnum;k++)
    pnbad=bdd_exist(pnbad,bdd_ithvar(k+bvar*varnum));
...
continue;
 ...
 { fprintf(fp,"\nM/T separition states   is:  t%d\n",i+1);
   for(k=0;k<bvar*varnum;k++)
        pnnew=bdd_exist(pnnew,bdd_ithvar(k+bvar*varnum));
